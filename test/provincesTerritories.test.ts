...
await describe('statscan-tools/provincesTerritories/lookups', async () => {
  await it('should have a clean set of alpha codes and aliases', () => {
    for (const [alphaCode, provinceTerritory] of Object.entries(
      provincesTerritories.provincesTerritories
    )) {
      assert.match(
        alphaCode,
        /^[A-Z]{2}$/v,
        `Province/Territory alpha code ${alphaCode} is not two capital letters`
      )
      ...
      assert.strictEqual(
        ...
        false,
        `Province/Territory ${provinceTerritory.name} has an unnecessary alias that matches its name`
      )
    }
  })
...
await describe('statscan-tools/provincesTerritories/utilities', async () => {
  await it('should find provinces/territories by abbreviation', () => {
    ...
    assert.ok(
      results !== undefined,
      'Expected to find a result for ON, but found none'
    )
  })
  ...
  await it('should find provinces/territories by name', () => {
    ...
    assert.ok(
      results !== undefined,
      'Expected to find a result for Ontario, but found none'
    )
  })
  ...
  await it('should find provinces/territories by alias', () => {
    ...
    assert.ok(
      results !== undefined,
      'Expected to find a result for Québec, but found none'
    )
  })

  await it('should return undefined if no matches are found', () => {
    const results = provincesTerritories.findProvinceTerritory(
  ...
  await it('should validate correct province/territory abbreviations', () => {
    assert.ok(
      provincesTerritories.isProvinceTerritoryAbbreviation('ON'),
      'Expected ON to be a valid province/territory abbreviation'
    )
  })

  await it('should invalidate incorrect province/territory abbreviations', () => {
    assert.ok(
      !provincesTerritories.isProvinceTerritoryAbbreviation('XX'),
      'Expected XX to be an invalid province/territory abbreviation'
    )
  })
})
